    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-7 gap-y-1">
        {cells.map((day, i) => {
          ...
          const base =
            'flex items-center justify-center h-8 w-full text-sm rounded-lg select-none transition-colors '
...
export default function CalendarEmbed() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-48 bg-white rounded-2xl shadow-md">
        <p className="text-gray-400 text-sm animate-pulse">Loading availability...</p>
      </div>
    )
  }
    ...
    <div className="bg-white rounded-2xl shadow-md p-6">
      <div className="flex items-start gap-2">
        <button
          onClick={prev}
          disabled={!canGoPrev}
          className="mt-1 p-1.5 rounded-lg hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      ...
      <div className="flex gap-5 mt-5 pt-4 border-t border-gray-100 text-sm text-gray-600">
